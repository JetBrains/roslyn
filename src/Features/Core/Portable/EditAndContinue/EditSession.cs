        ...
        internal Task<ImmutableArray<Document>> GetChangedDocumentsAsync(Solution solution, CancellationToken cancellationToken)
        {
            ...
            return Task.Factory.StartNew(async () =>
            {
                var tasks = solution.Projects.Select(async project =>
                {
                    ...
                    using var dummyDisposer = ArrayBuilder<Document>.GetInstance(out var dummy);
                    await PopulateChangedAndAddedDocumentsAsync(baseSolution, project, changedDocuments, dummy, cancellationToken);
