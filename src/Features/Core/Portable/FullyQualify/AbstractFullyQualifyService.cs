    ...
    where TSimpleNameSyntax : SyntaxNode
{
        ...
        Document document, TextSpan span, bool hideAdvancedMembers, CancellationToken cancellationToken)
    {
        ...
        using (Logger.LogBlock(FunctionId.Refactoring_FullyQualify, cancellationToken))
        {
            ...
            var matchingAttributeTypes = inAttributeContext ? await FindAsync(name + nameof(Attribute), ignoreCase, SymbolFilter.Type).ConfigureAwait(false) : new();
            var matchingNamespaces = inAttributeContext ? new() : await FindAsync(name, ignoreCase, SymbolFilter.Namespace).ConfigureAwait(false);
