...
{
    /// <summary>
    /// Provides a set of initialization methods for instances of the <see cref="FrozenSet4Hack{T}"/> class.
    /// </summary>
    public static class FrozenSet
    {
        /// <summary>Creates a <see cref="FrozenSet4Hack{T}"/> with the specified values.</summary>
        ...
        public static FrozenSet4Hack<T> ToFrozenSet4Hack<T>(this IEnumerable<T> source, IEqualityComparer<T>? comparer = null) =>
            GetExistingFrozenOrNewSet(source, comparer, out HashSet<T>? newSet) ??
            CreateFromSet(newSet!);

        /// <summary>Extracts from the source either an existing <see cref="FrozenSet4Hack{T}"/> instance or a <see cref="HashSet{T}"/> containing the values and the specified <paramref name="comparer"/>.</summary>
        private static FrozenSet4Hack<T>? GetExistingFrozenOrNewSet<T>(IEnumerable<T> source, IEqualityComparer<T>? comparer, out HashSet<T>? newSet)
        {
            comparer ??= EqualityComparer<T>.Default;

            // If the source is already frozen with the same comparer, it can simply be returned.
            if (source is FrozenSet4Hack<T> fs && fs.Comparer.Equals(comparer))
            {
                newSet = null;
                return fs;
            }
            ...
            if (newSet.Count == 0)
            {
                return ReferenceEquals(comparer, FrozenSet4Hack<T>.Empty.Comparer) ?
                    FrozenSet4Hack<T>.Empty :
        ...
        private static FrozenSet4Hack<T> CreateFromSet<T>(HashSet<T> source)
        {
            ...
            if (typeof(T).IsValueType && ReferenceEquals(comparer, EqualityComparer<T>.Default))
            {
                if (source.Count <= Constants.MaxItemsInSmallValueTypeFrozenCollection)
                {
                    ...
                    {
                        return (FrozenSet4Hack<T>)(object)new SmallValueTypeComparableFrozenSet<T>(source);
                    }

                    // Otherwise, use an implementation optimized for a small number of value types using the default comparer.
                    return (FrozenSet4Hack<T>)(object)new SmallValueTypeDefaultComparerFrozenSet<T>(source);
                }
                ...
                {
                    return (FrozenSet4Hack<T>)(object)new Int32FrozenSet((HashSet<int>)(object)source);
                }
                ...
                (ReferenceEquals(comparer, EqualityComparer<T>.Default) || ReferenceEquals(comparer, StringComparer.Ordinal) || ReferenceEquals(comparer, StringComparer.OrdinalIgnoreCase)))
            {
                ...
                FrozenSet4Hack<string>? frozenSet = LengthBucketsFrozenSet.CreateLengthBucketsFrozenSetIfAppropriate(entries, stringComparer, minLength, maxLength);
                if (frozenSet is not null)
                {
                    return (FrozenSet4Hack<T>)(object)frozenSet;
                }
    ...
    /// <remarks>
    /// <see cref="FrozenSet4Hack{T}"/> is immutable and is optimized for situations where a set
    ...
    /// the remainder of the life of the application. <see cref="FrozenSet4Hack{T}"/> should only be initialized
    /// with trusted elements, as the details of the elements impacts construction time.
    /// </remarks>
    //[DebuggerTypeProxy(typeof(ImmutableEnumerableDebuggerProxy<>))]
    [DebuggerDisplay("Count = {Count}")]
    public abstract class FrozenSet4Hack<T> : ISet<T>,
#if NET
        IReadOnlySet<T>,
#endif
        IReadOnlyCollection<T>, ICollection
    {
        ...
        private protected FrozenSet4Hack(IEqualityComparer<T> comparer) => Comparer = comparer;

        /// <summary>Gets an empty <see cref="FrozenSet4Hack{T}"/>.</summary>
        public static FrozenSet4Hack<T> Empty { get; } = new EmptyFrozenSet<T>(EqualityComparer<T>.Default);
        ...
        /// <summary>Enumerates the values of a <see cref="FrozenSet4Hack{T}"/>.</summary>
