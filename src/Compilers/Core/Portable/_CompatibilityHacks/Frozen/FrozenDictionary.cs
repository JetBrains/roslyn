...
namespace System.Collections.Frozen
{
    ...
    public static class FrozenDictionary
    {
        ...
        public static FrozenDictionary<TKey, TSource> ToFrozenDictionary<TSource, TKey>(
            ...
            where TKey : notnull => ToFrozenDictionary(source.ToDictionary(keySelector, comparer), comparer);
        ...
        public static FrozenDictionary<TKey, TElement> ToFrozenDictionary<TSource, TKey, TElement>(
            ...
            where TKey : notnull =>
            ToFrozenDictionary(source.ToDictionary(keySelector, elementSelector, comparer), comparer);
            ...
            where TKey : notnull
        {
            ...
            if (typeof(TKey).IsValueType && ReferenceEquals(comparer, EqualityComparer<TKey>.Default))
            {
                if (source.Count <= Constants.MaxItemsInSmallValueTypeFrozenCollection)
                {
                    ...
                    if (Constants.IsKnownComparable<TKey>())
                    {
                        return (FrozenDictionary<TKey, TValue>)
                            (object)new SmallValueTypeComparableFrozenDictionary<TKey, TValue>(source);
                    }

                    // Otherwise, use an implementation optimized for a small number of value types using the default comparer.
                    return (FrozenDictionary<TKey, TValue>)
                        (object)new SmallValueTypeDefaultComparerFrozenDictionary<TKey, TValue>(source);
                }
                ...
                {
                    return (FrozenDictionary<TKey, TValue>)(object)new Int32FrozenDictionary<TValue>(
                 ...
                 ReferenceEquals(comparer, StringComparer.OrdinalIgnoreCase)))
            {
                ...
                FrozenDictionary<string, TValue>? frozenDictionary =
                    LengthBucketsFrozenDictionary<TValue>.CreateLengthBucketsFrozenDictionaryIfAppropriate(keys, values,
                        stringComparer, minLength, maxLength);
                ...
                KeyAnalyzer.AnalysisResults analysis = KeyAnalyzer.Analyze(keys,
                    ReferenceEquals(stringComparer, StringComparer.OrdinalIgnoreCase), minLength, maxLength);
                if (analysis.SubstringHashing)
                {
                        ...
                        if (analysis.IgnoreCase)
                        {
                            frozenDictionary = analysis.AllAsciiIfIgnoreCase
                                ? new OrdinalStringFrozenDictionary_RightJustifiedCaseInsensitiveAsciiSubstring<TValue>(
                                    keys, values, stringComparer, analysis.MinimumLength, analysis.MaximumLengthDiff,
                                    analysis.HashIndex, analysis.HashCount)
                                : new OrdinalStringFrozenDictionary_RightJustifiedCaseInsensitiveSubstring<TValue>(keys,
                                    values, stringComparer, analysis.MinimumLength, analysis.MaximumLengthDiff,
                                    analysis.HashIndex, analysis.HashCount);
                        }
                        else
                        {
                            frozenDictionary = analysis.HashCount == 1
                                ? new OrdinalStringFrozenDictionary_RightJustifiedSingleChar<TValue>(keys, values,
                                    stringComparer, analysis.MinimumLength, analysis.MaximumLengthDiff,
                                    analysis.HashIndex)
                                : new OrdinalStringFrozenDictionary_RightJustifiedSubstring<TValue>(keys, values,
                                    stringComparer, analysis.MinimumLength, analysis.MaximumLengthDiff,
                                    analysis.HashIndex, analysis.HashCount);
                        }
                    }
                    else
                    {
                        if (analysis.IgnoreCase)
                        {
                            frozenDictionary = analysis.AllAsciiIfIgnoreCase
                                ? new OrdinalStringFrozenDictionary_LeftJustifiedCaseInsensitiveAsciiSubstring<TValue>(
                                    keys, values, stringComparer, analysis.MinimumLength, analysis.MaximumLengthDiff,
                                    analysis.HashIndex, analysis.HashCount)
                                : new OrdinalStringFrozenDictionary_LeftJustifiedCaseInsensitiveSubstring<TValue>(keys,
                                    values, stringComparer, analysis.MinimumLength, analysis.MaximumLengthDiff,
                                    analysis.HashIndex, analysis.HashCount);
                        }
                        else
                        {
                            frozenDictionary = analysis.HashCount == 1
                                ? new OrdinalStringFrozenDictionary_LeftJustifiedSingleChar<TValue>(keys, values,
                                    stringComparer, analysis.MinimumLength, analysis.MaximumLengthDiff,
                                    analysis.HashIndex)
                                : new OrdinalStringFrozenDictionary_LeftJustifiedSubstring<TValue>(keys, values,
                                    stringComparer, analysis.MinimumLength, analysis.MaximumLengthDiff,
                                    analysis.HashIndex, analysis.HashCount);
                        }
                    }
                ...
                else
                {
                    ...
                    {
                        frozenDictionary = analysis.AllAsciiIfIgnoreCase
                            ? new OrdinalStringFrozenDictionary_FullCaseInsensitiveAscii<TValue>(keys, values,
                                stringComparer, analysis.MinimumLength, analysis.MaximumLengthDiff, lengthFilter)
                            : new OrdinalStringFrozenDictionary_FullCaseInsensitive<TValue>(keys, values,
                                stringComparer, analysis.MinimumLength, analysis.MaximumLengthDiff, lengthFilter);
                    }
                    else
                    {
                        frozenDictionary = new OrdinalStringFrozenDictionary_Full<TValue>(keys, values, stringComparer,
                            analysis.MinimumLength, analysis.MaximumLengthDiff, lengthFilter);
                    }
                }
    ...
    public abstract class FrozenDictionary<TKey, TValue> : IDictionary<TKey, TValue>, IReadOnlyDictionary<TKey, TValue>,
        IDictionary
        where TKey : notnull
    {
        ...
        public void CopyTo(Span<KeyValuePair<TKey, TValue>> destination)
        {
            ...
            {
                throw new Exception("Destination is too small");
            }
        ...
        void ICollection.CopyTo(Array array, int index)
        {
            if (array.Rank != 1)
            {
                throw new ArgumentException("RankMultiDimNotSupported {0}", nameof(array));
            }

            if (array.GetLowerBound(0) != 0)
            {
                throw new ArgumentException("NonZeroLowerBound {0}", nameof(array));
            }
            ...
            {
                throw new ArgumentException("ArrayPlusOffTooSmall {0}", nameof(array));
            }
            ...
            else
            {
                ...
                {
                    throw new ArgumentException("IncompatibleArrayType {0}", nameof(array));
                }
        ...
        object? IDictionary.this[object key]
        {
            get
            {
                return key is TKey tkey && TryGetValue(tkey, out TValue? value) ? value : (object?)null;
            }
        ...
        IEnumerator<KeyValuePair<TKey, TValue>> IEnumerable<KeyValuePair<TKey, TValue>>.GetEnumerator() =>
            Count == 0
                ? ((IList<KeyValuePair<TKey, TValue>>)Array.Empty<KeyValuePair<TKey, TValue>>()).GetEnumerator()
                : GetEnumerator();

        /// <inheritdoc />
        IEnumerator IEnumerable.GetEnumerator() =>
            Count == 0 ? Array.Empty<KeyValuePair<TKey, TValue>>().GetEnumerator() : GetEnumerator();
